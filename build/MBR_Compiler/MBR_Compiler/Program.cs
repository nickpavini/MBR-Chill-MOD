...
namespace MbrCompiler
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //7Zip
            Console.WriteLine("Start to Zip");
            string version = getVersion();
            runTool("7z.exe", @"a -x!*.7z -x!*.zip -x!*.snk -x!*.lnk -xr!BackUp -xr!SmartFarm -x!*.sqlite3 -x!.git -x!Profiles -x!build*.bat -x!build -x!build\* -x!installer -x!installer\* -x!OLD*CODE -x!Zombies -x!Zombies\* -x!SkippedZombies -x!SkippedZombies\* -x!lib\*Debug* -x!*Debug*.txt -x!BuildMBR* -x!lib\*Debug*\* -x!*debug-txt -x!lib\*.txt -x!lib\*.html  -r MyBot-MBR_" + version  + ".zip *", false);
            goto Finish;
