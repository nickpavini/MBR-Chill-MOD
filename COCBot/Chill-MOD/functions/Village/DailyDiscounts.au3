...
Func DailyDiscounts()
	...
	For $i=0 To UBound($aDealsResult, 1)-1
		...
		If getOcrAndCapture("coc-freemagicitems", $aOcrPositions[$iSlot][0], $aOcrPositions[$iSlot][1], 80, 25, True) = "FREE" Then ContinueLoop ; ignore free deals
	...
	For $i=0 To UBound($aDeals, 1)-1
		If $g_abChkDD_Deals[$aDeals[$i][5]] = True Then
			...
			EndIf
			$g_iGemAmount -= $aDeals[$i][4] ; update gem amount after purchase
			_Sleep(1200)
		EndIf
	Next
